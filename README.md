...
// echanger listvalx(k=1) et listvalz(k=3) mieux reparti sur wiko
	listval[k].add(n,event.values[2-(k-1)]);
// sur samsung	
//	listval[k].add(n,event.values[k-1]);
	}
	if(n>0){	
	...
	for(int k=0;k<4;k++){
	if(k==0){
// precision temporelle wiko 5 chiffres,100000000+val/1000 pour 8e decimale	
	listdiff[k].add(p,(long)100000000+(long)listval[k].get(n)/1000-(long)listval[k].get(n-1)/1000);
// sur samsung
//	listdiff[k].add(p,(long)listval[k].get(n)-(long)listval[k].get(n-1));
	tdiff.setT(listdiff[k].get(p));
	}
	else{
// resolution accelerometre wiko 2 chiffres,val*0.1f pour 3e decimale
	listdiff[k].add(p,(float)listval[k].get(n)*0.1f-(float)listval[k].get(n-1)*0.1f);
// sur samsung
...
//	sttuni="T"+Integer.toString(p+1)+" qM"+Integer.toString(qmax)+" qa"+String.format("%.1f",(float)somq/(nq))+" q"+Integer.toString(q)+" r"+Integer.toString(r)+" t"+Float.toString((float)(event.timestamp-ti)/1000000000);	
	sttuni=(int)signef+"T"+String.format("%.1f",(float)(event.timestamp-ti)/1000000000);
	if(plein==false){	
	te=0;
	tj=0;	
	finlec=false;	
...
//	langue=Locale.FRANCE;	
	if(instalangue==true)langue=languedef;
	instalangue=false;
